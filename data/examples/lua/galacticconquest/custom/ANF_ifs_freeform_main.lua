...
--		end
	...
	SaveMissionSetup = function(this)
        ...
        -- add reinforcement counts
        missionSetup.reinforcements = 50 --50 will be default value
	...
	-- set up team colors (for ANF)
	ANFInitTeamColor = function(this)
		-- precomputed colors
		local colorWhite = { r=255, g=255, b=255 }
		local colorBlue = { r=0, g=0, b=255 }
	...
	GetCreditsColor = function(this, afford)
		if afford then
			return 255, 255, 0		-- can afford: yellow
		else
			return 255, 0, 0		-- can't afford: red
		end
	end,
...
--		SetProperty("gal_prp_grid", "GeometryColor", color)
		...
		for planet, _ in pairs(this.planetTeam) do
			...
			if planet ~= "pol" and planet ~= "dea" and planet ~= "tantive" then	-- Hack for ANF to get rid of that system error
...
--		this.planetPort,
	...
	OneTimeInit = function(this, showLoadDisplay)
		...
		print("ANF:GC: Our Scenario var is: ", this.scenario)
		print("This var determines if this should behave as stock or as an ANF GC")
		...
		if this.scenario == "ANF" then
			-- Hack for ANF; Read our models before theirs!
			ReadDataFile("..\\..\\Addon\\ZZZ\\data\\_LVL_PC\\ANF-GC-Models.lvl")
			print("ANF:GC:Main: ANF GC detected! Loading our models...")
		end
		...
		if this.scenario == "ANF" then
			-- Hack for ANF; Read our icons after theirs!
			ReadDataFile("..\\..\\Addon\\ZZZ\\data\\_LVL_PC\\ANF-GC-Textures.lvl")
			print("ANF:GC:Main: ANF GC detected! Loading our textures...")
		end
			-- read the galaxy map sounds
			...
			if this.scenario == "ANF" then
				this.setupSides = (this.custom ~= 1) -- nil (Hack for ANF)
				print("ANF:GC:Main: Hack for side selection ON")
			else
				this.setupSides = (this.custom ~= nil) -- Use stock settings for other GCs
				print("ANF:GC:Main: Hack for side selection OFF")
			end
			
			-- initialize team colors
			if this.scenario == "ANF" then
				this:ANFInitTeamColor()
				print("ANF:GC:Main: Using ANF colors!")
			else
				this:InitTeamColor()
				print("ANF:GC:Main: Using stock colors!")
			end
		end
	end,
	...
	Enter = function(this, bFwd)
		...
		if bFwd then
			...
			elseif this.setupSides then
				...
				-- Hack for ANF
				this.teamName[1] = "ifs.freeform.teams.all"
				this.teamName[2] = "ifs.freeform.teams.imp"
