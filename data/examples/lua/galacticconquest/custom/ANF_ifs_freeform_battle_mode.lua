...
ifs_freeform_battle_mode = NewIFShellScreen {
	...
	Enter = function(this, bFwd)
		...
		if ifs_freeform_main.scenario == "ANF" then
			-- skip if there is only one mode (for ANF)
			local count = 0
			for mode, mission in pairs(this.modes) do
				count = count + 0 --1
			end
			print("ANF:GC: Mode selection screen skipped!")
			if count <= 1 then
				this:Input_Accept()
				return
			end
		else
			-- skip if there is only one mode (for stock)
			local count = 0
			for mode, mission in pairs(this.modes) do
				count = count + 1 
			end
			print("ANF:GC: Mode selection screen Kept!")
			if count <= 1 then
				this:Input_Accept()
				return
			end
